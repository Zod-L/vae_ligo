...
from data import folder_dataset
...
def main(rank, world_size, batch, start_epoch, epoch, data_path, out_path, save_epoch, pretrain_path, \
    ...
    dataset = folder_dataset(data_path, threshold)
    sampler = DistributedSampler(dataset=dataset, num_replicas=world_size, rank=rank)
    dataloader = DataLoader(dataset=dataset, batch_size=batch, shuffle=False, sampler=sampler)
    ...
    for epoch in range(start_epoch, epoch+1):
        ...
        if epoch % save_epoch == 0:
            ...
            with torch.no_grad():
                ...
                pred, _, _ = model(im)
                res = torch.cat([im, thresh_im, pred], dim=0)
            model.train()

            if rank == 0:
                save_image(res, f"{out_path}/{epoch}.png", nrow=len(test_samples), normalize=True, value_range=(-1, 1))
                torch.save(model.module.state_dict(), f"{out_path}/{epoch}.pth")
        dist.barrier()
        ...
        for im, _ , msk in pbar:
            ...
            loss_percept = precept_weight * loss_fn_alex(F.interpolate(pred, 64), F.interpolate(im, 64)).mean()
...
if __name__ == "__main__":
    ...
    batch = 8
    epoch = 200
    data_path = "../gravityspy/processed/sub_2.0/"
    out_path = "./checkpoints/sub_2.0/1231-1231-4-5-128-threshold0-percept"
    pretrain_path = None
    save_epoch = 20
    start_epoch = 0
